...
def create_index(dataset_path, gpu=False, save=False):
    ...
    return index_ivf, image_names, feature_vectors


def create_index_array(dataset_path, gpu=False, save=False):
    """
    The dataset path leads to a pkl file of the form
    taht is a list of list, where a internal list is a feature vector
    If a GPU is available, send True
    This is used for the testing made to obtain the times
    """
    # 1. Opens the file and extracts the data
    with open(dataset_path, "rb") as file:
    ...
    return index_ivf, feature_vectors
...
def search_time(index, query, k):
    ...
    This search returns the elapsed time
    """
    # Preprocess and normalize the query vector
    query_vector = np.array(query)
    query_vector = query_vector.astype("float32")
